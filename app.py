...
# Hardcoded API Keys
GEMINI_API_KEY = "AIzaSyCqYnNDRPBG-DdXqxvbgLY85PRzEjcFF4o"  # Replace with your Gemini API key
GIPHY_API_KEY = "ueh0apsVu61b9JM5bVZsJkVh3tBMtjfK"  # Replace with your Giphy API key

# Configure Gemini API
genai.configure(api_key=GEMINI_API_KEY)

# List of space-themed emojis
space_emojis = ["🚀", "🌌", "🪐", "🔭", "🌠", "👨‍🚀", "🛰️", "🌍", "✨", "🌙"]

# Function to get a random space-related GIF from Giphy
def get_space_gif():
    try:
        url = f"https://api.giphy.com/v1/gifs/random?api_key={GIPHY_API_KEY}&tag=space&rating=g"
...
def chat():
    ...
    if not user_message:
        return jsonify({"response": "Hello, explorer! Ask me anything about space! 🚀"})

    # Define system instructions for a space-themed chatbot
    system_instruction = """You are COSMICA, a space exploration assistant., a chatbot dedicated to space exploration, astronomy, 
    astrophysics, and the mysteries of the universe. Always provide informative and engaging answers 
    about space, rockets, planets, stars, galaxies, black holes, astronauts, and space missions.Also mainly focus on indian space news and anything relate to it.
    If asked about unrelated topics, gently steer the conversation back to space."""
    ...
    try:
        ...
        if response.candidates and response.candidates[0].finish_reason == 1:
            response_text = response.text + " " + random.choice(space_emojis)
        else:
            response_text = "I'm not sure about that. Let's talk about space! 🚀"

        # Get a space-related GIF
        space_gif = get_space_gif()

        return jsonify({
            "response": response_text,
            "gif": space_gif  # Send GIF URL in response
        })
    except Exception as e:
        print("Error generating Gemini response:", e)
        return jsonify({"response": "Houston, we have a problem! Try again. 🚀"})
